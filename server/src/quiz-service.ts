...
class QuizService {
  ...
  updateUserWithNewQuiz(quizId, quiz) {
    return db
      ...
      .doc(quiz.creator)
      .update({
        quizes: _firestore.FieldValue.arrayUnion({
          name: quiz.name,
          quizid: quizId,
        }),
      });
  ...
  updateQuiz(newQuestions, quizId: string) {
    return db
      .collection("quizes")
      .doc(quizId)
  ...
  deleteQuizInUsers(userId: string, quizObject) {
  ...
  async addUserToQuizRoom(token: { i: string }, data, roomId) {
    try {
      const isAllowed = await adminAuth.verifyIdToken(token.i);
      if (data.uid === isAllowed.uid) {
        // Don't add quizmaster to his own quiz
        return null;
      }
      if (isAllowed.uid) {
  ...
  async changeQuizRoomRound(token: { i: string }, roomId) {
    try {
      ...
      if (isAllowed.uid) {
        ...
        if (
          Object.keys(roomData.quiz.questions).length <
          Number(roomData.round + 1)
        ) {
          await roomRef.update({
            finished: true,
            inProgress: false,
